...
1. 我的个人信息分析：
   - 成绩定位，包括考生姓名、成绩、选科组合、民族、户籍、学费接受上限等等
   - 地域偏好，分析考生信息中意向地域
   - 限制条件，分析考生信息中报考限制选择

2. 核心诉求：
   - 结合考生成绩、意向专业、意向地域和院校专业策略等进行综合分析
...
请用专业但易懂的语言进行解读，重点分析为什么这套志愿方案适合我的个人情况，以及方案核心价值。解读应避免过于琐碎的细节，聚焦在方案的整体价值和战略考量上。过程中不显示院校的录取分数，不显示院校分数范围，不需要输出优化建议和总结。规避掉一本、二本等字眼。不显示分差安全边际。避免零滑档的说法。
"""

ANALYZING_CATEGORY_PROMPT = """
作为高考志愿规划专家，请按照如下规则对冲刺志愿、稳妥志愿或保底志愿三个层次的志愿进行专业、全面但简明的分析：
1. 整体配置分析
   - 请说明院校专业组数量配比合理性
   - 分析各层级间的院校梯度设置特点
   - 指出志愿方案中对我意向专业覆盖最具优势的配置亮点

2. 院校选择策略与分析：
   - 院校专业组的选择策略，以及院校与考生意向相关的特色说明。
   - 院校的地域分布特点

3. 专业选择策略与分析：
   - 专业选择策略，专业与考生需求的匹配度，只分析考生意向专业。
   - 专业就业前景,只分析优势专业。

请用专业但易懂的语言进行解读，既专业又实用，避免泛泛而谈，针对我提供的志愿方案中的具体院校和专业进行个性化分析。过程中不显示院校的录取分数，不显示院校分数范围，不需要输出优化建议和总结。规避掉一本、二本等字眼。不显示分差安全边际。避免零滑档的说法。
"""
