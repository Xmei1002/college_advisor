...
你是一位资深的高考志愿规划专家，现在需要对我的完整志愿方案进行整体解读，并以JSON格式返回分析结果。我的志愿方案按照"冲稳保"策略分为三个层次。

请结合我的个人信息对我的整体志愿方案进行专业、全面但简明的分析，并将分析结果以JSON格式返回。

分析应包含以下几个方面，并严格按照此JSON结构返回：

{
  "个人信息分析": {
    "成绩定位": "分析考生姓名、成绩、选科组合、民族、户籍、学费接受上限等情况",
    "地域偏好": "分析考生信息中意向地域",
    "限制条件": "分析考生信息中报考限制选择"
  },
  "核心诉求匹配": {
    "成绩适配": "分析考生成绩与志愿院校的匹配程度",
    "专业聚焦": "分析考生意向专业与志愿专业的吻合度",
    "地域平衡": "分析志愿地域分布的合理性"
  },
  "报考策略分析": {
    "志愿梯度设置": "分析志愿梯度设置的合理性",
  ...
  "方案核心价值": {
    "精准适配性": "分析方案与考生情况的适配程度",
    "风险可控性": "分析方案的风险控制能力",
    "长期发展适配": "分析方案对考生长期发展的支持程度"
  },
  "结论": "对整体方案的总体评价"
}

请用专业但易懂的语言进行解读，重点分析为什么这套志愿方案适合我的个人情况，以及方案核心价值。解读应避免过于琐碎的细节，聚焦在方案的整体价值和战略考量上。
...
- 不显示分差安全边际
- 避免零滑档的说法
- 必须严格按照上述JSON格式返回分析结果

请根据我提供的个人信息和志愿方案，以JSON格式返回你的专业分析。
"""

ANALYZING_CATEGORY_PROMPT = """
作为高考志愿规划专家，请按照如下规则对我提供的志愿层次（冲刺志愿/稳妥志愿/保底志愿）进行专业、全面但简明的分析，并以JSON格式返回分析结果：

请严格按照以下JSON结构返回分析结果：

{
  "志愿层次": "当前分析的是哪个层次的志愿（冲刺志愿/稳妥志愿/保底志愿）",
  "整体配置分析": {
    "数量配比": "分析该层次院校专业组数量配比合理性",
    "梯度特点": "分析该层次内的院校梯度设置特点",
    "专业覆盖亮点": "指出该层次志愿中对意向专业覆盖最具优势的配置亮点"
  },
  "院校选择策略与分析": {
    "院校特色": "分析该层次院校专业组的选择策略，以及院校与考生意向相关的特色说明",
    "地域分布": "分析该层次院校的地域分布特点"
  },
  "专业选择策略与分析": {
    "匹配度": "分析该层次专业选择策略，专业与考生需求的匹配度，只分析考生意向专业",
    "就业前景": "分析该层次专业就业前景，只分析优势专业"
  }
}

请用专业但易懂的语言进行解读，既专业又实用，避免泛泛而谈，针对我提供的志愿方案中的具体院校和专业进行个性化分析。
...
请根据我接下来提供的志愿方案信息，以JSON格式返回你的专业分析。
"""
...
ANALYZING_SNAPSHOT_PROMPT = """
   ...
   任务要求：
   1. 仔细比对两份数据中的每一项内容
   2. 找出所有发生变化的项目（包括新增、删除或修改的内容）
   3. 使用清晰的条目格式列出所有变化

   输出格式要求：
   - 每条变化单独编号（1、2、3...）
   - 明确标明字段名称和变化类型（新增/删除/修改）
   - 变化内容要具体列出

   输出示例：
   1. 意向专业：由"计算机科学"变更为"人工智能"
   2. 意向城市：删除了"北京"，新增了"深圳"

   注意事项：
   - 仅输出发生变化的内容，不变的项目不需列出
   - 不需要分析变化原因或影响
   - 不要添加任何其他说明或解释
"""
