...
from app.api.schemas.preference import (
    ...
    CollegePreferenceFullSchema,
    CollegePreferenceResponseSchema
)
...
def create_college_preference(data):
    ...
    保存学生的志愿填报意向信息（地域、学费、专业、学校及报考限制等）
    """
    # 获取当前用户
    ...
    # 处理自定义梯度数量
    if 'custom_gradient_counts' in data and data['custom_gradient_counts']:
        if preference and preference.volunteer_gradient_strategy == '自由设置':
            # 如果是JSON字符串，转换为字典
            if isinstance(data['custom_gradient_counts'], str):
                try:
                    data['custom_gradient_counts'] = json.loads(data['custom_gradient_counts'])
                except:
                    current_app.logger.error("自定义梯度数量格式错误")
                    data['custom_gradient_counts'] = None
